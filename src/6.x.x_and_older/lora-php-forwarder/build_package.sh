#!/bin/bash
# =============================================================================
# build_package.sh — lora-php-forwarder packaging script (mPower 6.x.x)
#
# Usage:
#   ./build_package.sh
#
...
#   2. Checks that the python3-requests .ipk is present in provisioning/
#   3. Creates a .tar.gz ready to upload on mPower via Apps > Custom Apps
#
# Output:
#   lora-php-forwarder_<version>.tar.gz  (in the current directory)
#
# Prerequisites:
#   - The python3-requests .ipk file must be placed in provisioning/ first.
#     See README.md section "Obtenir le fichier python3-requests .ipk"
# =============================================================================
...
# ---------------------------------------------------------------------------
MISSING=0
for f in manifest.json Start Install forwarder.py config/forwarder.cfg.json provisioning/p_manifest.json; do
    if [ ! -f "$f" ]; then
...
if [ "$IPK_COUNT" -eq 0 ]; then
    ...
    echo ""
    echo "  The python3-requests .ipk must be placed in provisioning/ before packaging."
    echo "  Expected filename: python3-requests_2.27.1-1_arm926ejste.ipk"
    echo ""
    echo "  To obtain it, download it from the OpenWRT repository and rename it:"
    echo "    wget https://downloads.openwrt.org/releases/21.02.7/packages/arm_arm926ej-s/packages/python3-requests_2.27.1-1_arm_arm926ej-s.ipk"
    echo "    mv python3-requests_*.ipk provisioning/python3-requests_2.27.1-1_arm926ejste.ipk"
    echo ""
